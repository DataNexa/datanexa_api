...
const createQueryMonitoramentos = (ativos:boolean) => {
    ...
    let query = `
        ...
        gsc.sites,
        gsc.noInSites,                   -- coluna renomeada no novo schema
        gsc.inUrl,
        gsc.inTitle,
        gsc.inText,
    ...
    query += `  LEFT JOIN google_search_config             gsc ON gsc.monitoramento_id = m.id AND gsc.client_id = m.client_id
                ...
                LEFT JOIN index_hashtags_instagram         ihi ON ihi.hashtag_value    = ish.hashtag_value 
...
export default {
    ...
    async getMonitoramentosDeClientesAtivosEConfigs(): Promise<MonitoramentoFull[] | false> {
        ...
        try {
             ...
             * ----------------------------------------------------------------- */
            const queryMonitoramentos = await conn.query(createQueryMonitoramentos(true))
