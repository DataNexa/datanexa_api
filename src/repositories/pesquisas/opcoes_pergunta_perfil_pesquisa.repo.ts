...
interface opcoes_pergunta_perfil_pesquisa_i {
    id:number,
    pergunta_perfil_pesquisa_id:number,
    valor:string
}
...
interface unique_response {
    ...
    message:string,
    code:number,
    row?:opcoes_pergunta_perfil_pesquisa_i
}

const opcoes_pergunta_perfil_pesquisa_repo = {
        
    
    list: async (pergunta_perfil_pesquisa_id:number,pesquisa_id:number,client_id:number, injectString:string=''):Promise<opcoes_pergunta_perfil_pesquisa_i[]|false> => {
        ...
        ${injectString}`, {
            binds:[pergunta_perfil_pesquisa_id,pesquisa_id,client_id]
        })
    ...
    create: async (pergunta_perfil_pesquisa_id:number,valor:string, client_id:number):Promise<create_response> => {
        ...
        VALUES (?,?)
         `, {
            binds:[pergunta_perfil_pesquisa_id,valor]
        })
    ...
    update: async (pergunta_perfil_pesquisa_id:number,valor:string,pesquisa_id:number,client_id:number,id:number):Promise<boolean> => {
        
        const resp = await execute(`update opcoes_pergunta_perfil_pesquisa      
        join perguntas_perfil_pesquisa on opcoes_pergunta_perfil_pesquisa.pergunta_perfil_pesquisa_id = perguntas_perfil_pesquisa.id 
         join pesquisas on perguntas_perfil_pesquisa.pesquisa_id = pesquisas.id 
         join client on pesquisas.client_id = client.id 
      set  opcoes_pergunta_perfil_pesquisa.pergunta_perfil_pesquisa_id = ?,  opcoes_pergunta_perfil_pesquisa.valor = ?
         WHERE  perguntas_perfil_pesquisa.id = ? and  pesquisas.id = ? and  client.id = ?  and opcoes_pergunta_perfil_pesquisa.id = ? `, {
            binds:[pergunta_perfil_pesquisa_id,valor,pergunta_perfil_pesquisa_id,pesquisa_id,client_id,id]
        })
...
export { opcoes_pergunta_perfil_pesquisa_repo, opcoes_pergunta_perfil_pesquisa_i }