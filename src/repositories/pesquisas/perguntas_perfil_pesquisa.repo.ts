...
const perguntas_perfil_pesquisa_repo = {
    ...
    list: async (pesquisa_id:number,client_id:number, injectString:string=''):Promise<perguntas_perfil_pesquisa_i[]|false> => {
        ...
        SELECT  perguntas_perfil_pesquisa.id,  perguntas_perfil_pesquisa.pesquisa_id,  perguntas_perfil_pesquisa.pergunta
        ...
        return (resp.rows as perguntas_perfil_pesquisa_i[])
    },    
    ...
    unique: async (pesquisa_id:number,client_id:number,id:number):Promise<unique_response> =>  {
        ...
        from perguntas_perfil_pesquisa 
         ...
         WHERE  pesquisas.id = ? and  client.id = ? 
            and perguntas_perfil_pesquisa.id = ? `, {
        ...
        const rows = (resp.rows as perguntas_perfil_pesquisa_i[])
